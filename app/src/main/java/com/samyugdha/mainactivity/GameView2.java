...
public class GameView2 extends View {
    ...
    private  static final int COLS=6,ROWS=8;
    private static final float WALL_THICKNESS=4;
    private float cellSize,hMargin,vMaring;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        vMaring=(height-ROWS*cellSize)/2;
        canvas.translate(hMargin,vMaring);
        for(int x=0;x<COLS;x++)
        {
            for(int y=0;y<ROWS;y++)
            {
        ...
        canvas.drawRect(player.col*cellSize+margin,player.row*cellSize+margin,(player.col+1)*cellSize-margin,
                (player.row+1)*cellSize-margin,playerPaint);
        canvas.drawRect(exit.col*cellSize+margin,exit.row*cellSize+margin,(exit.col+1)*cellSize-margin,
                (exit.row+1)*cellSize-margin,exitPaint);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(event.getAction()==MotionEvent.ACTION_MOVE)
        {
            ...
            float playerCenterY=vMaring+(player.row+0.5f)*cellSize;
